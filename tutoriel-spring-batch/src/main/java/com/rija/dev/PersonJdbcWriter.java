...
/**
 * ItemWriter pour pérenniser les données qui ont été traitées via l'ItemProcessor
 * @author rija
 *
 */
...
public class PersonJdbcWriter implements ItemWriter<Personne> {
	...
	public void write(List<? extends Personne> items) throws Exception {
		for (Personne personne : items) {
			final Object object[] = { personne.getNom(), personne.getPrenom(),
					personne.getCivilite(), personne.getId() };
			// on tente un update
			...
			if (nbLigne == 0) {
				final Object object2[] = { personne.getId(), personne.getNom(),
						personne.getPrenom(), personne.getCivilite() };
